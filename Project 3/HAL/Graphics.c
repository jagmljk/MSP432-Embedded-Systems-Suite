 ...
 * Graphics.c
 *
 * Implementation of LCD graphics functions using TI's grlib library.
 */
...
int GFX_printTextRows(GFX* gfx_p, char* strings[], int numStrings, float firstRow, float col) {
    ...
    for (i = 0; i < numStrings; i++) {
        GFX_print(gfx_p, strings[i], firstRow++, col);
    }
    return firstRow;
}
...
{
    uint32_t oldForeground = gfx_p->foreground;
    GFX_setForeground(gfx_p, gfx_p->background);
    GFX_drawSolidRectangle(gfx_p, x1, x2, y1, y2);
    GFX_setForeground(gfx_p, oldForeground);
}
